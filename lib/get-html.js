...
@font-face {
    ...
    // Process LaTeX math expressions with code block protection
    let processedMd = md;
    ...
    // Protect code blocks first (both ``` and ` styles)
    ...
    // Restore protected blocks (avoid recursive replacement)
    for (let i = protectedBlocks.length - 1; i >= 0; i--) {
        const placeholder = `__PROTECTED_BLOCK_${i}__`;
        processedMd = processedMd.split(placeholder).join(protectedBlocks[i]);
    }
