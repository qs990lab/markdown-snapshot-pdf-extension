 ...
 */
module.exports = async (md, config) => {
    ...
    if (hasLocalMermaid) {
        ...
        try {
            ...
            mermaidScript = `<script src="https://cdn.jsdelivr.net/npm/mermaid@11.12.0/dist/mermaid.min.js"></script>`;
        }
    } else {
...
@font-face {
    ...
    // Extract and protect code blocks (both ``` and ` styles)
    ...
    // Process inline math ($...$) - now safe from code blocks
    processedMd = processedMd.replace(/\$([^$\n]+)\$/g, (match, math) => {
        try {
            return katex.renderToString(math.trim(), {
                displayMode: false,
    ...
    for (let i = 0; i < codeBlocks.length; i++) {
        ...
        const inlinePlaceholder = `__INLINE_CODE_${i}__`;
        processedMd = processedMd.replace(placeholder, codeBlocks[i]);
        processedMd = processedMd.replace(inlinePlaceholder, codeBlocks[i]);
    }
...
<head>
	...
	<style>
		...
		/* Better spacing for chemical formulas and mathematical expressions */
		sub + sup, sup + sub {
			margin-left: -0.1em;
		}
...
${await (await getMarked(config.marked_options))(processedMd)}
