...
const { extractHeadings, addPdfOutline } = require('./add-pdf-outline');
...
module.exports = async(mdFile, config, port, args = {}) => {
    ...
    // Set document title from filename if not specified
    if (!config.title) {
        const filename = path.basename(mdFile, path.extname(mdFile));
        config.title = filename;
    }

    // Handle one-page configuration
    if (config.onePage) {
        // Add body class for one-page styling
        config.body_class = [...(config.body_class || []), 'one-page'];
        
        console.log('One-page PDF mode enabled');
    }
    ...
    // Pass config directly (not spread) so modifications are preserved
    config.port = port;
    ...
    // Add PDF outline (TOC)
    // - Always for one-page PDFs
    // - For multi-page PDFs only if no Mermaid diagrams (accurate positioning)
    if (!config.as_html && output.filename && output.headingPositions) {
        const hasMermaid = md.includes('```mermaid');
        const isOnePage = output.pdfPageHeight !== null && output.pdfPageHeight !== undefined;
        
        if (isOnePage || !hasMermaid) {
            const headings = extractHeadings(md);
            await addPdfOutline(
                output.filename, 
                headings, 
                output.headingPositions, 
                output.pdfPageHeight, 
                output.pdfPageHeights,
                output.pdfMarginTop,
                output.headingPageNumbers
            );
        }
    }
