 ...
 */
async function addPdfOutline(pdfPath, headings, positions = {}) {
    ...
    try {
        ...
        for (const heading of headings) {
            ...
            if (positions[heading.id]) {
                // Convert HTML Y (from top) to PDF Y (from bottom)
                // Account for margins (15mm = ~42.5 points)
                const pixelToPoint = 0.75; // 1px = 0.75pt
                const topMargin = 42.5;
                yPos = pageHeight - (positions[heading.id].y * pixelToPoint) - topMargin;
            }
