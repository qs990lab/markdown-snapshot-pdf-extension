...
module.exports = async options => {
    ...
    marked.use({ extensions: [displayMathExtension, inlineMathExtension, highlightTextExtension, subscriptExtension, superscriptExtension, highlightExtension] });
    
    // Configure marked options (remove highlight function)
    marked.setOptions({
        gfm: true,
        breaks: false,
        pedantic: false,
    ...
    // Add custom renderer for headings with IDs and name anchors for PDF
    const renderer = {
        heading({ text, depth }) {
            const textStr = typeof text === 'string' ? text : text.text || '';
            // Generate ID that preserves Unicode characters (Japanese, Chinese, etc.)
            let id = textStr
                .toLowerCase()
            ...
            // Handle duplicate IDs (same logic as add-pdf-outline.js)
            ...
            // Add both id and name for PDF anchor support
            return `<h${depth} id="${id}" name="${id}">${textStr}</h${depth}>`;
        }
    };
    ...
    return function(src, opts, callback) {
        ...
        return marked(src, opts, callback);
    };
};